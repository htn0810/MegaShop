...
const Cart = () => {
  ...
  const calculateTotalPriceAndQuantity = () => {
    const totalPrice = storedCart?.cartProductsGroupByShop.reduce((acc, group) => {
      return (
        acc +
        group.products.reduce((acc, product) => acc + (product.isChecked ? product.price * product.quantity : 0), 0)
      )
    }, 0)
    const totalQuantity = storedCart?.cartProductsGroupByShop.reduce((acc, group) => {
      return acc + group.products.reduce((acc, product) => acc + (product.isChecked ? product.quantity : 0), 0)
    }, 0)
  ...
  useEffect(() => {
    if (storedCart) {
      setCart({
        cartProducts: storedCart.cartProductsGroupByShop?.map((group: ProductsGroup) => group.products).flat(),
        totalQuantity: storedCart.totalQuantity,
        totalPrice: storedCart.totalPrice,
        id: storedCart.id,
        userId: storedCart.userId,
      })
      setProductsGroup(storedCart.cartProductsGroupByShop)
      setIsLoadingCart(true)
      setTimeout(() => {
        setIsLoadingCart(false)
      }, 1000)
    ...
    if (storedCart?.totalQuantity) {
      setCart((prev) => {
        ...
        return {
          ...
          totalQuantity: storedCart.totalQuantity,
          totalPrice: storedCart.totalPrice,
          cartProducts: storedCart.cartProducts,
        }
      })
      setProductsGroup(storedCart.cartProductsGroupByShop)
    }
  }, [storedCart?.totalQuantity])
      ...
      {!isLoadingCart && cart && productsGroup && productsGroup.length > 0 && (
        <section className='py-6'>
          {productsGroup.map((group) => (
            <CartProductsGroup key={group.shopId + generateDynamicId()} shop={group} />
          ))}
